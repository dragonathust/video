...
BEGIN_MESSAGE_MAP(CVGA2USBApp, CWinApp)
	...
	ON_COMMAND(ID_NETSERVICE_ENABLE,OnNetserviceEnable)
...
void CVGA2USBApp::GetWindowConfig()
{
	...
	port=::GetPrivateProfileInt("NETWORK CONFIG","port",5000,strConfigFileName); 
...
void CVGA2USBApp::OnNetserviceEnable()
{
	if(!m_bNetServiceStart)
	{
	((CMainFrame* )m_pMainWnd)->SetButtonState(ID_NETSERVICE_ENABLE,TBSTATE_PRESSED);	
	HWND hHwnd = ((CMainFrame* )m_pMainWnd)->GetSafeHwnd();
	int V2U_NetServerStart(HWND hHwnd);
		if(V2U_NetServerStart(hHwnd))
		{
		m_bNetServiceStart=true;
		}
		else
		{
			((CMainFrame* )m_pMainWnd)->SetButtonState(ID_NETSERVICE_ENABLE,TBSTATE_ENABLED);
		}
	}
	else
	{
