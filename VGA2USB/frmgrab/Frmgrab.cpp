...
#include "include\frmgrab.h"
#include "include\v2u_lib.h"
#include "include\v2u_version.h"
#include "avi_writer.h"

void mprintf( char* format,... );
#define v2u_println mprintf
#define v2u_error mprintf
...
/*
 * Writes raw file to the specified stream.
 */
static V2U_BOOL v2u_write_raw(FILE * out, const void * pixels, size_t framelen)
{
    if (fwrite(pixels, 1, framelen, out) < framelen) {
        return V2U_FALSE;
    }
...
static V2U_BOOL v2u_save_frame(const V2U_GrabFrame2* frame, const char* fname)
{
    ...
    if (out) {
        ...
        if (save_image) {
            ...
            result = v2u_write_raw(out, frame->pixbuf, frame->imagelen);
        }
            ...
            if (AW_FInit(&aw, out, vm->width, vm->height) == AW_OK) {
                ...
                if (AW_AddVideoStream(aw, formatInfo->fourcc, bpp) == AW_OK &&
                    ...
                    while (frames < ctx->frameCount) {
                        ...
                        if (AW_AddFrame(aw, 0, (unsigned char*)frame->pixbuf, frame->imagelen,
...
static int v2u_list_cards()
{
    ...
    if (n > 0) {
        FrmGrabber** fg = (struct _FrmGrabber ** )malloc(sizeof(FrmGrabber*)*n);
        if (fg) {
...
static void v2u_usage()
{
    ...
    printf("  %s, %s METHOD   resize to crop rectangle using specified "
        "algorithm (%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s)\n",
 ...
 */
int v2u_main(int argc, char * argv[])
{
