/****************************************************************************
 *
 * $Id: v2u_util.c 11808 2010-12-07 15:55:23Z monich $
 *
...
{
    const V2U_BYTE* y = src;
    const V2U_BYTE* v = y + w*h;
    const V2U_BYTE* u = v + w*h/4;
...
{
    const V2U_BYTE* y = src;
    const V2U_BYTE* u = y + w*h;
    const V2U_BYTE* v = u + w*h/4;
    v2u_convert_planar_yuv_to_rgb24(dst, y, u, v, row, w, h); 
}
    ...
    int w, int h)
{
    ...
    default:
        srcRow = src;
        srcRow += (V2UPALETTE_2_BPP(fmt)*w/8)*row;
