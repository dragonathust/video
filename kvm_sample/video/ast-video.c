...
int VideoCapture (PVIDEO_ENGINE_INFO VideoEngineInfo)
{
	int Status;
	u32 data;
...
#if 1
		ast_video_reset();
		//system("echo 1 > /sys/devices/platform/apb/1e700000.video/video_reset"); 
...
//		printf("send host header \n");
		if( send (connfd, &Transfer_Header, 29, MSG_WAITALL) <= 0) {
				return -1;
		};
		//recv client header
		do {
...
WAIT_CONNECT:
	...
	while(!do_exit) {
		if( VideoCapture(&VideoEngineInfo) < 0 )
			break;
	}
